...
#define sqlite4_snprintf snprintf
...
int lsmSortedWalkFreelist(
  ...
  if( rc==LSM_OK ){
    ...
    while( rc==LSM_OK && lsmMCursorValid(pCsr) && rtIsSystem(pCsr->eType) ){
      void *pKey = NULL; int nKey;
      void *pVal = NULL; int nVal;
...
void sortedDumpPage(lsm_db *pDb, Segment *pRun, Page *pPg, int bVals){
  ...
  for(i=0; i<nRec; i++){
    ...
    int iChar;
    u8 *aKey = NULL; int nKey = 0;       /* Key */
    u8 *aVal = NULL; int nVal = 0;       /* Value */
    int iTopic;
    u8 *aCell;
    int iPgPtr;
    int eType;
...
static void infoCellDump(
  ...
  u8 *aData = NULL; int nData;           /* Page data */
  u8 *aKey = NULL; int nKey = 0;         /* Key */
  u8 *aVal = NULL; int nVal = 0;         /* Value */
  int eType;
  int iPgPtr;
